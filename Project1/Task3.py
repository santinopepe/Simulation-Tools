...
def Solver_BDF2(k=1.0, t_end=20.0, h=0.02, nout=2000, y0=None):
    ...
    if y0 is None:
        y0 = np.array([1.1, 1.1 , 0.0, 0.0], dtype=float)
...
def Solver_EE(k=1.0, t_end=20.0, h=0.02, nout=2000, y0=None):
    """Solver using Assimulo Explicit Euler with fixed step size."""
    rhs = ElasticPendulumRHS(k)

    if y0 is None:
        y0 = np.array([1.1, 1.1 , 0.0, 0.0], dtype=float)

    prob = Explicit_Problem(rhs, y0, name=f"Elastic pendulum Explicit Euler (k={k})")
...
def plot_comparison(k=1.0, t_end=50.0, h=0.02, nout=2000):
    ...
    te, ye, sime = Solver_EE(k=k, t_end=t_end, h=0.000001, nout=nout, y0=y0)
    
    # Extract components
    y1_2, y2_2, y3_2, y4_2 = y2.T
    y1_4, y2_4, y3_4, y4_4 = y4.T
    y1_e, y2_e, y3_e, y4_e = ye.T
    ...
    ax1.plot(y1_e, y2_e, 'g-.', label='Explicit Euler', linewidth=2, alpha=0.8)
    ax1.set_xlabel('y1', fontsize=12)
    ax1.set_ylabel('y2', fontsize=12)
    ax1.set_title(f'Trajectory (y1, y2)', fontsize=13, fontweight='bold')
    ...
    # Plot 2: Position vs time (single figure, all methods)
    fig2, ax2 = plt.subplots(figsize=(12, 6))
    # Plot all methods with the same style as the reference
    ax2.plot(t2, y2_2, 'b--', label='BDF-2: y2 (y)', linewidth=1.8, alpha=0.7, zorder=1)
    ax2.plot(t4, y1_4, 'r-', label='BDF-4: y1 (x)', linewidth=2.4, alpha=0.9, zorder=2)
    ax2.plot(t4, y2_4, 'r--', label='BDF-4: y2 (y)', linewidth=2.4, alpha=0.9, zorder=2)
    # Draw BDF-2 y1 before EE so green is on top
    ax2.plot(t2, y1_2, 'b-', label='BDF-2: y1 (x)', linewidth=2.6, alpha=0.95, zorder=3)
    ax2.plot(te, y1_e, 'g-', label='Explicit Euler: y1 (x)', linewidth=2.6, alpha=1.0, zorder=4)
    ax2.plot(te, y2_e, 'g--', label='Explicit Euler: y2 (y)', linewidth=2.6, alpha=1.0, zorder=4)
    ax2.set_xlabel('Time (s)', fontsize=12)
    ax2.set_ylabel('Position', fontsize=12)
    ax2.set_title('Positions vs Time', fontsize=13, fontweight='bold')
...
if __name__ == "__main__":
    ...
    plot_comparison(k=1000.0, t_end=30.0, h=0.001, nout=2000)
